...
!     ************
    ...
    double precision :: theta,fold,dr,rr,tol,dpmeps, &
...
!              R_z is the upper triangular part of S'ZZ'Y.
    ...
    if (updatd) then
        ...
        !                                 shift old part of WN1.
            do 10 jy = 1, m - 1
                js = m + jy
                call dcopy1(m-jy,wn1(jy+1,jy+1),wn1(jy,jy))
                call dcopy1(m-jy,wn1(js+1,js+1),wn1(js,js))
                call dcopy1(m-1,wn1(m+2,jy+1),wn1(m+1,jy))
            10 END DO
        endif
...
!        upper triangle of (2,2) block of wn.
    ...
    do 72 is = col+1, col2
        do 74 js = is, col2
            wn(is,js) = wn(is,js) + ddot1(col,wn(1,is),wn(1,js))
        74 END DO
    72 END DO
...
!     Update matrices WS and WY.
    ...
    call dcopy1(n,d,ws(1,itail))
    call dcopy1(n,r,wy(1,itail))
...
!                                         and the lower triangle of SY:
    ...
    !                              move old information
        do 50 j = 1, col - 1
            call dcopy1(j,ss(2,j+1),ss(1,j))
            call dcopy1(col-j,sy(j+1,j+1),sy(j,j))
        50 END DO
    endif
...
!                                             and the last column of SS:
    pointr = head
    do 51 j = 1, col - 1
        sy(col,j) = ddot1(n,d,wy(1,pointr))
        ss(j,col) = ddot1(n,ws(1,pointr),d)
        pointr = mod(pointr,m) + 1
    51 END DO
    if (stp == one) then
        ss(col,col) = dtd
...
!        solve trans(t)*x=b for t lower triangular.
    ...
    do 90 jj = 2, n
        j = n - jj + 1
        b(j) = b(j) - ddot1(jj-1,t(j+1,j),b(j+1))
        b(j) = b(j)/t(j,j)
    90 END DO
    100 continue
    go to 140
