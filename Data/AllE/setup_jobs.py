...
def const_set():
    ...
    for ks in Kmax:
        yield {"ks":ks, "kw": ks, "kww": ks, "els":4, "elw":4}
...
def setup_all():
    ...
    update_params = rec_io.parse_params("Template/Template/inputs/parameters.txt")
    params = rec_io.update_param_vals(default_params, update_params)
    param_list = []
#   for i, const in enumerate(const_set()):

    Kmax = np.round(10.**np.linspace(1, 5, 300), 2)
    E = np.arange(0.25, 25.25, 0.25)

    for i, e in enumerate(E):
        for j, ks in enumerate(Kmax):
            const = {"ks":ks, "kw": ks, "kww": ks, "els":e, "elw":e}
